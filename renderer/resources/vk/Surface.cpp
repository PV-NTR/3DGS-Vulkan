...
std::shared_ptr<Surface> Surface::Make(std::shared_ptr<RenderPass> renderPass,
...
{
    attachmentResources_.emplace_back(VkResourceManager::GetInstance().GetImageManager()
        .RequireImage({ width_, height_, format, depthStencil }));
...
void Surface::CreateAttachmentResources()
{
    ...
    for (auto format : formats) {
        attachmentResources_.emplace_back(VkResourceManager::GetInstance().GetImageManager()
            .RequireImage({ width_, height_, format, i++ == renderPass_->DepthStencilAttachmentID()}));
    }
}
